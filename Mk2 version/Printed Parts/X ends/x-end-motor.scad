...
module x_end_motor(){
 difference(){
     ...
     union(){
            x_end_motor_base();
translate([0,-2.7,0])            x_end_motor_endstop_base();
         }
  x_end_motor_shaft_cutout();
  x_end_motor_holes();
translate([0,-2.7,0])  x_end_motor_endstop_holes();    
      ...
      //#translate([-16,-40,0])cube([50,100,100]);   
                      // Bottom pushfit rod
translate(v=[-15,-31.2,6]) rotate(a=[-90,0,0]) pushfit_rod(7.9,41);
// Top pushfit rod
translate(v=[-15,-31.2,45+6]) rotate(a=[-90,0,0]) pushfit_rod(7.9,41);
       translate([0,-20.7,10])   vertical_bearing_holes();  
 }
