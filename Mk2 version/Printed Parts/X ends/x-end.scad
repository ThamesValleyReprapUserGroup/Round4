...
// Main block
height = 58;
translate(v=[-15,-10.8,height/2]) cube(size = [17,40.8,height], center = true);
//# Bearing holder
 translate([0,-20.7,0])vertical_bearing_base();	
//Nut trap
 //# Cylinder
   translate(v=[0,0.3,0]) poly_cylinder(h = 8, r=12.5, $fn=25);
 // Hexagon
...
module x_end_holes(){
// #
translate([0,-20.7,0])   vertical_bearing_holes();
// Belt hole
translate(v=[-1,0,0]){
// Stress relief
...
// Bottom pushfit rod
translate(v=[-15,-41.5,6]) rotate(a=[-90,0,0]) pushfit_rod(7.9,50);
// Top pushfit rod
translate(v=[-15,-41.5,rod_distance+6]) rotate(a=[-90,0,0]) pushfit_rod(7.9,50);

// TR Nut trap
   //# Hole for the nut
    translate(v=[0,0.3, -1]) poly_cylinder(h = 9.01, r = 4, $fn = 25);
